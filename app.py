...
# 🌟 СИГУРНО СПАСЯВАНЕ И ВЪЗСТАНОВЯВАНЕ ЧРЕЗ LOCALSTORAGE (БЕЗ ПРЕЗАГРЕЖДАНЕ)
if not session_id:
    # Записваме текущите стойности в браузъра, докато потребителят ги променя
    js_save = f"""
    <script>
    ...
    # Когато се връщаме от Stripe, използваме скрит JS компонент, който само изпраща обратно 
    # стойностите към Streamlit, без да пренасочва или презарежда целия прозорец!
    js_retrieve = """
    <script>
    const fc = localStorage.getItem('fc') || '1200';
    const pr = localStorage.getItem('pr') || '50';
    const cs = localStorage.getItem('cs') || '20';
    const idea = localStorage.getItem('idea') || '';
    
    // Изпращаме данните обратно към Streamlit прозореца по сигурен начин
    window.parent.postMessage({
        type: 'streamlit:setComponentValue',
        value: {fc: fc, pr: pr, cs: cs, idea: idea}
    }, '*');
    </script>
    """
    # Извикваме компонента и улавяме стойността му директно в Python
    with st.container():
        browser_data = components.html(js_retrieve, height=0)
        
        # Ако в URL адреса има вече преки параметри, ги взимаме с приоритет
        url_fin = st.query_params.get("fin", "")
        url_idea = st.query_params.get("idea", "")
        
        if url_fin and "|" in url_fin and "{CLIENT_" not in url_fin:
            try:
                parts = url_fin.split("|")
                if len(parts) == 3:
                ...
                pass
        # Ако липсват в URL (заради новия таб на Stripe), но сме ги записали в session_state преди плащането:
        # Streamlit ще запази текущите стойности, въведени в слайдерите, без да забие сайта.
        ...
        if not api_key:
            ...
            with st.spinner("AI Менторът изчислява финансовия риск..."):
                try:
                    ...
                    dynamic_url = f"{stripe_link}?fin={fin_payload}&idea={encoded_idea}"
