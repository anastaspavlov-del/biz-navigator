...
# 📥 УЛАВЯНЕ НА ДИНАМИЧНИТЕ ДАННИ ДИРЕКТНО ОТ URL
...
# Парсване на първоначалните стойности от URL
...
if session_id:
    with st.spinner("🔒 Проверка на статуса на плащането..."):
        ...
        # За презастраховане в тестова среда: ако има session_id, го приемаме за валиден
    ...
    if not api_key:
        ...
        with st.spinner("🤖 Експертният AI консултант анализира ТВОЯТА идея..."):
            try:
                ...
                st.download_button(
                    label="📥 Изтегли Бизнес плана в редактируем Word (.docx) format",
                    data=docx_bytes,
